...
unzFile ZEXPORT unzOpenBuffer (const  void* buffer, uLong size)
{
    ...
    ourmemory_t* unzmem = (ourmemory_t*)malloc(sizeof(ourmemory_t));

    unzmem->size = size;
    unzmem->base = (char *)buffer;

    fill_memory_filefunc64_32(&zlib_filefunc64_32_def_fill, unzmem);
    return unzOpenInternal("__notused__", &zlib_filefunc64_32_def_fill, 0);
}
